...
const contractAddress = "0xA2d885dc62F3396b0Eda0f5b034Fb9d914c93584";
...
const dApp = {
  ...
  collectVars: async function() {
    // get Planetoid tokens
    this.tokens = [];
    this.totalSupply = await this.AlderaanContract.methods.totalSupply().call();
    ...
    for (let i = 1; i <= this.totalSupply; i++) {
      try {
        const token_uri = await this.AlderaanContract.methods.tokenURI(i).call();
        console.log('token uri', token_uri)
        const token_json = await fetchMetadata(token_uri);
        console.log('token json', token_json)
        this.tokens.push({
          tokenId: i,
          highestBid: Number(await this.AlderaanContract.methods.highestBid(i).call()),
          auctionEnded: Boolean(await this.AlderaanContract.methods.auctionEnded(i).call()),
          pendingReturn: Number(await this.AlderaanContract.methods.pendingReturn(i, this.accounts[0]).call()),
          auction: new window.web3.eth.Contract(
            this.auctionJson,
            await this.AlderaanContract.methods.auctions(i).call(),
            { defaultAccount: this.accounts[0] }
          ),
          owner: await this.AlderaanContract.methods.ownerOf(i).call(),
          ...token_json
        });
      } catch (e) {
  ...
  bid: async function(event) {
    ...
    await this.AlderaanContract.methods.bid(tokenId).send({from: this.accounts[0], value: wei}).on("receipt", async (receipt) => {
  ...
  endAuction: async function(event) {
    const tokenId = $(event.target).attr("token-id");
    await this.AlderaanContract.methods.endAuction(tokenId).send({from: this.accounts[0]}).on("receipt", async (receipt) => {
      M.toast({ html: "Transaction Mined! Refreshing UI..." });
      await this.updateUI();
    });
  },
  ...
  registerPlanetoid: async function() {
    ...
    try {
      ...
      await this.AlderaanContract.methods.registerPlanetoid(reference_uri).send({from: this.accounts[0]}).on("receipt", async (receipt) => {
        M.toast({ html: "Transaction Mined! Refreshing UI..." });
  ...
  main: async function() {
    ...
    this.planetoidJson = await (await fetch("./MosEisleyMarket.json")).json();
    this.auctionJson = await (await fetch("./Pieces_of_Alderaan.json")).json();

    this.AlderaanContract = new window.web3.eth.Contract(
      this.planetoidJson,
      this.contractAddress,
      { defaultAccount: this.accounts[0] }
    );
    console.log("Contract object", this.AlderaanContract);

    this.isAdmin = this.accounts[0] == await this.AlderaanContract.methods.owner().call();
